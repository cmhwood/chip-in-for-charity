...
router.get("/admin", rejectUnauthenticated, (req, res) => {
  const queryText = `SELECT * FROM "restrictions" ORDER BY "id";`;
  pool
    .query(queryText)
    .then((result) => {
...
router.post("/admin", rejectUnauthenticated, async (req, res) => {
  ...
  try {
    const sqlText = `INSERT INTO "restrictions" ("name")
    VALUES ($1);`;
...
router.put("/admin/:id", rejectUnauthenticated, async (req, res) => {
  console.log("/restrictions/admin/:id PUT route");
  ...
  try {
    const result = await pool.query(
      `
        UPDATE restrictions SET "name" = $1
...
router.delete("/admin/:id", rejectUnauthenticated, async (req, res) => {
  pool
    .query('DELETE FROM "restrictions" WHERE id=$1', [req.params.id])
    .then((result) => {
      res.sendStatus(200);
    })
    .catch((error) => {
      console.log("Error DELETE /api/restrictions/:id", error);
      res.sendStatus(500);
    });
});
